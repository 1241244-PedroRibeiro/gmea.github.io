...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    foreach ($detailsArray as $detail) {
        ...
        $checkStmt = $mysqli->prepare("SELECT COUNT(*) AS count FROM pautas_avaliacao WHERE userOuTurma = ? AND disciplina = ? AND semestre = ?");
        ...
        if ($checkData['count'] != 0) {
            ...
            $insertStmt = $mysqli->prepare("INSERT INTO pautas_avaliacao (id_pauta, user, disciplina, par1, par2, par3, notas, prof, userOuTurma, semestre, nivel, escala, media) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        }
    }        
    ...
    if ($insertStmt) {
        ...
        foreach ($detailsArray as $detail) {
            ...
            // Verificar se já existe um registro com os mesmos critérios
                ...
                $insertStmt->bind_param("isiiiisssiiii", $id_pauta, $user, $disciplina, $aproveitamento, $atitudes, $empenho, $observacoes, $prof, $userOuTurma, $semestre, $nivel, $escala, $media);
                $insertStmt->execute();
        }
